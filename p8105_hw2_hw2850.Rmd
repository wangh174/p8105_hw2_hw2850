...
```{r} 
library(readxl)
mr_trash_wheel=read_excel("data/Trash-Wheel-Collection-Totals-7-2020-2.xlsx", "Mr. Trash Wheel", range = "A2:N535") %>% 
...
```{r}
professor_trash_wheel=read_excel("data/Trash-Wheel-Collection-Totals-7-2020-2.xlsx", "Professor Trash Wheel") %>% 
  janitor::clean_names(dat = .) %>% 
  drop_na(data=., dumpster) %>% 
  mutate(sports_balls=as.integer(sports_balls),
...
pols=read_csv(file = "./data/fivethirtyeight_datasets/pols-month.csv") %>% 
  ...
  select(.data = ., -c(prez_dem, prez_gop, day))
```
